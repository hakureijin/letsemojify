...
- **Keyboard** — All interactive elements (chapter chips, language toggle, decade index, range-zoom buttons, category chips, footer source links, cumulative-chart medallions, origin-map pins, origin-map zoom controls) carry `focus-visible:ring-2` styles in the appropriate accent color. Markers are reachable via Tab and activatable with Enter/Space; Escape dismisses any pinned tooltip. The origin-map SVG itself is focusable — `+ / −` zoom, `0` resets, and arrow keys pan when the map has focus.
